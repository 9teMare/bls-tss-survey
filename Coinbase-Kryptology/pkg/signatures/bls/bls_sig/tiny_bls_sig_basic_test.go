...
func TestBasicVtThresholdKeygen(t *testing.T) {
	...
	t.Logf(">>>> Tiny: Long PK, Short Signature")
	...
	loop := 100
	for i := 0; i < loop; i++ {
		signAndAggregateVt(t, bls, pk, int(threshold), sks)
	}
...
func signAndAggregateVt(t *testing.T, bls *SigBasicVt, pk *PublicKeyVt, threshold int, shares []*SecretKeyShare) {
	...
	signatures := make([]*PartialSignatureVt, threshold)
	...
	for worker := 0; worker < threshold; worker++ {
		go func(worker int) {
			sig, err := bls.PartialSign(shares[worker], digest)
			defer wg.Done()
	...
	_, err = bls.CombineSignatures(signatures...)
...
func generateRandomMsgVt(length int) ([]byte, error) {
...
func sha256DigestVt(b ...[]byte) []byte {
	hash := sha256.New()
	for _, bytes := range b {
		hash.Write(bytes)
	}
